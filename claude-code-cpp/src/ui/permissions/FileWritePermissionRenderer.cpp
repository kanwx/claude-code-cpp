...
Element FileWritePermissionRenderer::renderPrompt(const PermissionRequest& req,
                                                   const RenderContext& ctx) {
    auto details = renderDetailLines(req, ctx);
    Elements children;
    children.push_back(hbox({
        text("  Allow ") | ftxui::color(ctx.theme.warning) | bold,
        text("Write") | ftxui::color(ctx.theme.accent) | bold,
...
std::string FileWritePermissionRenderer::getActivityDescription(
    const PermissionRequest& req) {
    auto path = extractField(req.input, "file_path");
    return path.empty() ? req.activity : path;
}

std::vector<ftxui::Element> FileWritePermissionRenderer::renderDetailLines(
